...
import { useEffectAsync, useMemoAsync } from './hooks-async';
import {
  defaultStopTimeout,
  ffmpegCoreUrl,
...
export const useWhisper: UseWhisperHook = (config) => {
  ...
  const voiceSignal = useRef<number[]>([]);
  ...
  const onWhispered = useMemoAsync(
    async (file: File) => {
      ...
      let finalEndpoint = whisperApiEndpoint;
      if (customEndpoint !== undefined) {
        finalEndpoint = customEndpoint
      }
      const response = await axios.post( finalEndpoint + mode, body, {
        headers,
      })
